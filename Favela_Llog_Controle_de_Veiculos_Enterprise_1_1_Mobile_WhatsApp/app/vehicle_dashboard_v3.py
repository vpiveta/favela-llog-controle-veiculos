...
def _owner_borrow_alert():
    ...
    return DailyChecklist.query.options(
        ...
        DailyChecklist.vehicle_id == current_user.vehicle.id,
        ...
        (DailyChecklist.checklist_type == 'DEVOLUCAO').desc(),
...
def _fleet_fuel_status():
    if not current_user.is_authenticated or not current_user.is_admin:
        return []
    vq = Vehicle.query.options(selectinload(Vehicle.driver)).filter_by(vehicle_type='MOTORCYCLE').order_by(Vehicle.plate)
    vehicles = vq.all()
    if not vehicles:
        return []
    ids = [v.id for v in vehicles]
    max_dates = db.session.query(
        Expense.vehicle_id.label('vehicle_id'),
        func.max(Expense.expense_date).label('max_date'),
    ).filter(
    ...
    return [
        {
            ...
            'days': _fuel_age(latest.get(vehicle.id)),
        }
...
def vehicle_v3_context():
    ...
    return {
        'v3_vehicle': vehicle,
        'v3_last_return': last_return,
        'v3_last_return_km': last_return.odometer if last_return else (vehicle.current_km if vehicle else None),
        'v3_last_fuel': last_fuel,
        'v3_fuel_days': _fuel_age(last_fuel),
        'v3_borrow_alert': _owner_borrow_alert(),
        'v3_fleet_fuel': _fleet_fuel_status() if request.endpoint == 'main.dashboard' else [],
    }
...
def init_vehicle_dashboard_v3(app):
    ...
    def validate_checklist_odometer_against_last_return():
        ...
        if last_return and odometer < last_return.odometer:
            flash(
                f'O KM informado ({odometer}) não pode ser menor que a última devolução '
                f'({last_return.odometer} km em {last_return.checklist_date.strftime("%d/%m/%Y")}).',
                'danger',
            )
