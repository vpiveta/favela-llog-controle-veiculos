...
def _panel(c, x, y, w, h, radius=3*mm):
    c.setFillColor(colors.Color(0.02, 0.03, 0.035, alpha=.92))
...
def _label_value(c, label, value, x, y, vx, maxw):
    ...
    _fit(c, value, vx, y, maxw, 7.5, WHITE, 'Helvetica-Bold')
...
def _draw_photo(c, raw, x, y, w, h):
    ...
    try:
        ...
        buf.seek(0)
        c.drawImage(ImageReader(buf), x, y, w, h, preserveAspectRatio=False, mask='auto')
        return True
    except Exception:
        return False
...
def _status(c, value, x, y):
    u = value.upper()
    ok = 'OK' in u
    stroke = GREEN if ok else AMBER
    fill = colors.Color(.02,.20,.08,alpha=.95) if ok else colors.Color(.20,.12,.01,alpha=.95)
    c.setStrokeColor(stroke)
    c.setFillColor(fill)
    c.roundRect(x, y, 25*mm, 5.2*mm, 2.4*mm, fill=1, stroke=1)
    c.setFillColor(stroke)
    c.setFont('Helvetica-Bold', 6.2)
    c.drawCentredString(x+12.5*mm, y+1.55*mm, 'OK' if ok else 'ATENÇÃO')
...
def checklist_pdf(title, rows, filename):
    ...
    # Canto esquerdo totalmente livre para a marca do fundo.
    title_x = 70*mm
    c.setFillColor(colors.Color(0,0,0,alpha=.58))
    c.roundRect(title_x-4*mm, h-39*mm, 73*mm, 29*mm, 3*mm, fill=1, stroke=0)
    _txt(c, 'CHECKLIST', title_x, h-21*mm, 18, WHITE, 'Helvetica-Bold')
    _txt(c, 'DIÁRIO', title_x, h-29*mm, 17, TEAL, 'Helvetica-Bold')
    _txt(c, 'RETIRADA  /  DEVOLUÇÃO', title_x, h-35*mm, 7.5, WHITE, 'Helvetica-Bold')
    c.setStrokeColor(TEAL); c.setLineWidth(1.4); c.line(title_x, h-37*mm, title_x+55*mm, h-37*mm)
    ...
    _panel(c, margin, top_y, left_w, 38*mm)
    _panel(c, margin+left_w+gap, top_y, right_w, 38*mm)
    ...
    y = table_y+table_h-17*mm
    for label, value in items[:11]:
        c.setStrokeColor(colors.HexColor('#30383C')); c.line(margin+4*mm, y-1*mm, w-margin-4*mm, y-1*mm)
        _fit(c, label.upper(), margin+7*mm, y+1*mm, 55*mm, 6.5, WHITE, 'Helvetica-Bold')
        _status(c, value, margin+66*mm, y-1*mm)
        obs = value.split('-',1)[1].strip() if ('ATEN' in value.upper() and '-' in value) else ('Requer atenção.' if 'ATEN' in value.upper() else '---')
        _fit(c, obs, margin+99*mm, y+1*mm, 73*mm, 6.0, WHITE, 'Helvetica')
        y -= 6.6*mm

    # Fotos: usa somente miniaturas já disponíveis no banco durante a geração.
    # Isso evita bloquear o worker do Render baixando 4 imagens grandes do Storage,
    # que era a causa do 502/timeout. Fotos antigas que só estejam no Storage
    # continuam acessíveis na tela do checklist, mas não derrubam o PDF.
    ...
    photo_y = 91*mm
    _panel(c, margin, photo_y, content, 28*mm)
    ...
    for row in photos[:4]:
        ...
        raw = row.content if row.content else None
        drawn = _draw_photo(c, raw, px, photo_y+3*mm, pw, ph)
        if not drawn:
            _txt(c, 'Foto disponível no sistema', px+2*mm, photo_y+10*mm, 5.2, MUTED, 'Helvetica')
        _txt(c, cats.get(row.category,row.category.replace('CHECKLIST_','').title()), px, photo_y+1*mm, 5.4, MUTED, 'Helvetica-Bold')
        px += pw+gap2
    ...
    _txt(c,'Favela Llog · Controle de Veículos',margin,12*mm,6.2,TEAL,'Helvetica-Bold')
    _txt(c,'Documento gerado pelo sistema',w-63*mm,12*mm,5.8,MUTED)
    ...
    def wrapper(title, rows, filename):
        if str(title).lower().startswith('checklist'):
            try:
                ...
                # Nunca deixa um PDF customizado derrubar a aplicação.
