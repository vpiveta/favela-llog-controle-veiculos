...
def _issue_matches(issue, services_text):
    ...
    aliases = {
        'freio': ('freio', 'pastilha', 'sapata', 'disco'),
        'pneu': ('pneu', 'roda', 'câmara', 'camara'),
        'luz': ('luz', 'farol', 'lanterna', 'lâmpada', 'lampada'),
        'seta': ('seta', 'indicador'),
        'buzina': ('buzina',),
        'corrente': ('corrente', 'relação', 'relacao', 'coroa', 'pinhão', 'pinhao'),
        'retrovisor': ('retrovisor', 'espelho'),
        'óleo': ('óleo', 'oleo', 'lubrificante', 'filtro de óleo', 'filtro de oleo'),
        'suporte': ('suporte',),
        'baú': ('baú', 'bau', 'baga'),
    }
...
def apply_analysis(expense, analysis, actor_id=None):
    ...
    ai_km = analysis.get('odometer')
    try:
        ai_km = int(ai_km) if ai_km is not None else None
    except Exception:
        ai_km = None
    if ai_km is not None and ai_km >= 0:
        expense.odometer = max(expense.odometer or 0, ai_km)
        if ai_km > (vehicle.current_km or 0):
            vehicle.current_km = ai_km
            changed = True
    ...
    if expense.maintenance:
        ...
        extras = [service for service in services if service.casefold() not in lower_original]
        if extras:
            suffix = f"\n{AI_MARKER} Serviços adicionais identificados na nota: " + '; '.join(extras)
            detail.description = original.rstrip() + suffix
            changed = True

        if analysis.get('workshop') and not detail.workshop:
            detail.workshop = str(analysis.get('workshop')).strip()[:160] or None
            changed = True

        oil_detected = bool(analysis.get('oil_change')) and confidence >= 0.60
        if oil_detected:
            ...
            if base_km > 0 and not existing:
                db.session.add(OilChange(
                    change_date=expense.expense_date,
                    odometer=base_km,
                    next_change_km=base_km + 990,
        ...
        if services and confidence >= 0.60:
            ...
            open_issues = VehicleIssue.query.filter_by(vehicle_id=vehicle.id, status='OPEN').all()
            for issue in open_issues:
                if _issue_matches(issue, services_text):
                    issue.status = 'RESOLVED'
                    issue.resolved_at = utc_now()
                    issue.resolved_by_id = actor_id or expense.created_by_id
                    issue.maintenance_expense_id = expense.id
                    changed = True
    ...
    if AI_MARKER not in note:
        ...
        if summary:
            audit_note += f'. Resumo: {summary}'
        expense.notes = (note.rstrip() + ('\n' if note.strip() else '') + audit_note)[:10000]
        changed = True

    if changed and actor_id:
        db.session.add(AuditLog(
            action='AI_RECEIPT_SYNC', entity_type='EXPENSE', entity_id=expense.id,
            description=f'Nota analisada por IA e ficha do veículo {vehicle.plate} sincronizada.',
            user_id=actor_id,
        ))
    return changed
...
def process_expense(expense, actor_id=None, force=False):
    if not expense or expense.is_deleted or expense.asset_type != 'MOTORCYCLE':
        return False
    if not force and AI_MARKER in (expense.notes or ''):
        return False
...
def reprocess_existing(actor_id=None, limit=None):
    query = Expense.query.filter(
        ...
        Expense.expense_type.in_(('MAINTENANCE', 'FUEL')),
    ).order_by(Expense.expense_date.asc(), Expense.id.asc())
    processed = 0
    changed = 0
    for expense in query.all():
        if AI_MARKER in (expense.notes or ''):
            continue
        if limit is not None and processed >= limit:
            break
        processed += 1
        try:
            if process_expense(expense, actor_id=actor_id):
                changed += 1
        except Exception:
            db.session.rollback()
            current_app.logger.exception('Falha ao reprocessar nota da despesa %s', expense.id)
...
def init_receipt_ai_upgrade(app):
    ...
    def _sync_new_receipt(response):
        if (
            ...
            and request.endpoint in {'main.maintenance_new', 'main.fuel_new'}
            and current_user.is_authenticated
            and (os.getenv('OPENAI_API_KEY') or '').strip()
        ):
            previous = getattr(g, '_receipt_ai_previous_expense_id', 0)
            latest = Expense.query.filter(
                Expense.created_by_id == current_user.id,
                Expense.id > previous,
            ).order_by(Expense.id.desc()).first()
            if latest:
                try:
                    process_expense(latest, actor_id=current_user.id)
                except Exception:
                    db.session.rollback()
                    current_app.logger.exception('Falha na análise automática da nova nota %s', latest.id)
        return response

    @app.cli.command('reprocessar-notas-ia')
    def reprocessar_notas_ia():
        if not (os.getenv('OPENAI_API_KEY') or '').strip():
            print('OPENAI_API_KEY não configurada; nenhuma nota foi processada.')
            return
